...
public class AWSconnect2 extends AsyncTask<String, Void, String> {
    private TextView textView1;
    private TextView textView2;

    public AWSconnect2(TextView textView1, TextView textView2) {//textviewを設定する。複数をしたいなら複数分引数などの設定をして
        super();
    ...
    protected void onPostExecute(String result) {//すぐ上のaがresultに入る
        ...
        String[] test = result2.split("," , 0);//phpから受け取ったものを要素ごとに分ける
        int testlength = test.length;
        int i = 0;
        if(i < testlength) {
            textView1.setText(test[i]);//表示
            textView2.setText(test[2]);//表示
        }
