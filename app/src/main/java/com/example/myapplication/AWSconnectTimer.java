...
public class AWSconnectTimer extends AsyncTask<String, Void, String> {
    ...
    public AWSconnectTimer(TextView textView1 , TextView textView2 , TextView textView3 , TextView textView4, TextView textView5) {//textviewを設定する。複数をしたいなら複数分引数などの設定をして
        super();
    ...
    protected void onPostExecute(String result) {//すぐ上のaがresultに入る
        ...
        if(i < testlength) {
            int second = Integer.parseInt(test[i]);
            int minute = second / 60;
            second = second % 60;
            textView2.setText(String.valueOf(minute));//表示
            textView3.setText(String.valueOf(second));
            i++;
        }
        if(i < testlength) {
            int second = Integer.parseInt(test[i]);
            int minute = second / 60;
            second = second % 60;
            textView4.setText(String.valueOf(minute));//表示
            textView5.setText(String.valueOf(second));
        }
